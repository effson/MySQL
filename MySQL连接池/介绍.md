...
<img width="430" height="321" alt="image" src="https://github.com/user-attachments/assets/c00eefcb-2d2e-4fbf-b256-dc7d68e21406" />
...
## 6.裸MySQL语句和预处理MySQL语句
### 裸 SQL
直接把完整的 SQL 语句拼接好后交给 MySQL 执行，例如
...
### 预处理 SQL
SQL 语句先提交给 MySQL 编译解析，其中变量部分用占位符 ?，后续执行时只传递参数，例如
...
### 执行流程
裸 SQL：
- 每次执行都需要 SQL 解析 → 语法检查 → 执行计划生成 → 执行

<br>

预处理 SQL：
- 第一次执行时 SQL 解析与编译，之后仅需 传参并执行，避免了重复的解析过程

### 性能区别
- 裸 SQL：适合一次性执行的语句
- 预处理 SQL：适合需要 重复执行 的语句（如批量插入、循环查询），因为省去了多次解析和编译的开销

### 安全性
裸 SQL：
- 如果拼接字符串参数，容易遭受 SQL 注入攻击。

<br>

预处理 SQL：
- 参数与 SQL 语句分离，MySQL 内部会对参数进行转义和校验，天然防 SQL 注入
